...
#ifndef A1_MAZE_1_MODEL_H
#define A1_MAZE_1_MODEL_H
...
namespace s21 {
    ...
    public:
        ...
        BasicWrapper<T> Generate(const BasicGenerationSettings<T> &settings, Parameters &&...function_params) {
            return generator_device_.Generate(settings, std::forward<Parameters>(function_params)...);
        }

        template<FileTypes T, typename ...Parameters>
        std::vector<Point> Solve(const BasicWrapper<T> &data, Parameters && ...function_params) {
            return path_finder_device_.Solve(data, std::forward<Parameters>(function_params)...);
        }
