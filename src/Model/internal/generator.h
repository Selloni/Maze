...
namespace s21 {
     ...
     * Содержит в себе поля настроек для дальшейшей генерации
     *
     */
     ...
     */
    template <> struct BasicGenerationSettings<kCave> final {
     ...
     */
    template <> struct BasicGenerationSettings<kMaze> final {
    ...
    namespace internal {
         ...
         *
         * s21::Generator<s21::kCave>::Generate(settings)\n
         * s21::Generator<s21::kCave>::Generate(settings, cave)\n
         * s21::Generator<s21::kMaze>::Generate(settings)\n
         *
         * @tparam T FileTypes enum
         */
        template<FileTypes T> class Generator final {
            NON_CONSTRUCTABLE_CLASS(Generator)
        };

        template<> class Generator<kMaze> final {
        public:
            NON_CONSTRUCTABLE_CLASS(Generator);
             ...
             *
             * @param s настройки генерации габиринта
             *
             */
            static MazeWrapper Generate(const MazeGenerationSettings &s) {
                CaveWrapper vertical{std::vector<size_type>(s.rows * s.cols, size_type {}), s.rows, s.cols},
                                        horizontal{std::vector<size_type>(s.rows * s.cols, size_type{}), s.rows, s.cols};

                if (!vertical.IsGood() or !horizontal.IsGood())
                    return {};
                ...
                std::vector<size_type> random(s.rows * s.cols * 3, {});
                {
                    ...
                    std::generate(random.begin(), random.end(), [&distribution, &re] {
                ...
                auto Merge = [&](size_type i) {
                    ...
                    for (size_type col = 0; col != line.size(); ++col) {
                        if (line[col] == merged_item)
                            line[col] = line[i];
                    }
                };

                for (size_type row = 0; row != s.rows - 1; ++row) {
                    std::transform(line.begin(), line.end(), line.begin(), [&counter](size_type item) { return item == 0 ? counter++ : item; });
                    ...
                    for (size_type col = 0; col != line.size(); ++col) {
                        ...
                        if (choice == 1) {
                            ...
                            for (size_type c = 0; c != line.size(); ++c)
                                if (line[c] == line[col] and horizontal(row, c) == 0)
                                    count++;
                            if (count != 1)
                ...
                return {std::move(vertical.data), std::move(horizontal.data), s.rows, s.cols};
            }
        };

        template<> class Generator<kCave> final {
        public:
            NON_CONSTRUCTABLE_CLASS(Generator);

            /**
             *
             * Метод генерирует пещеру с начальной инициализацией GenerationSettings::live_chance.\n
             * Также отрабатывает GenerationSettings::generation_count поколений,\n
             * возвращает последнее поколение.\n
             * Пределы жизни и рождения задаются соответственно в GenerationSettings,\n
             * born_limit, GenerationSettings::live_limit) соответственно
             * @param s generation settings
             * @return generated cave
             *
             */
            static CaveWrapper Generate(const CaveGenerationSettings &s) {
                CaveWrapper cave{InitializeRandom(s.rows, s.cols, s.live_chance)}, tmp = cave;
             ...
             *
             */
            static CaveWrapper &Generate(const CaveGenerationSettings &s, CaveWrapper &cave) {
                size_type generation = 0;
        ...
        private:
            static CaveWrapper InitializeRandom(size_type rows, size_type cols, size_type live_chance) {
                std::default_random_engine re(std::chrono::system_clock::now().time_since_epoch().count());
            ...
            static size_type CountLiveNeighbours(size_type row, size_type col, const CaveWrapper &cave) {
                size_type count = 0;
